...
def sendImage(msg,s):
    ...
    print "1--------------->"+msg[0]
    print "1--------------->"+msg[1]
    f1 = open( msg[1], 'rb') #lee image (path)
    data = f1.read()
    c_f1 = data.encode('base64')#codifica
    size = len(c_f1)#agafa mida
    s.sendall('/image ' + str(size))
    time.sleep(1)
    print c_f1
    s.sendall('/image ' + c_f1)

# /image /home/daniels/Escriptori/Squirtle.png
def recv_image(EntryText,s):
    path ="/home/daniels/Escriptori/yeep.png"
    msg = EntryText.split()# /image + size

    img = open(path,'wb')
    r_size = 0

    data = s.recv(4096000)# Conte (Nom: /image ksfksfksbfksfk)
