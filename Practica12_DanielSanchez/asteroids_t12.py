...
#COLISIONES
def colliderShip():#NAVE CON ASTEROIDE
    for i in [x for x in world.sprites if isinstance(x, Asteroid)]:
        for f in [y for y in world.sprites if isinstance(y, Bullet)]:
            if ( abs(f.rect.center[0] - i.rect.center[0]) < 10 and abs(f.rect.center[1] - i.rect.center[1]) < 10):
                i.kill()
                f.kill()
                print "BUUUM"

def colliderShoot():#DISPARO CON ASTEROIDE
    for i in [x for x in world.sprites if isinstance(x, Asteroid)]:
        if ( abs(player.rect.center[0] - i.rect.center[0]) < 10 and abs(player.rect.center[1] - i.rect.center[1]) < 10):
...
def main():
    ...
    updateColision = Thread(target=update_collision)

    supdate.start()
    updateColision.start()
    while world.running:
